...
export default function ServerManagePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      getAuditLog(token, { server: decodedName, limit: 200 }),
    ])
      .then(
        ([
          infoResponse,
        ...
        setSelectedAudit((prev) => prev ?? auditResponse.items?.[0] ?? null);
      },
      )
      .catch((err) => {
      ...
      {loading ? (
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          >
            ...
            <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
              Track server actions like rewards edits, pushes, bans, wipes, and server status changes.
            </p>
            {auditError && (
              <div className="mt-3 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700 dark:border-red-900/40 dark:bg-red-900/20 dark:text-red-200">
                {auditError}
              </div>
            )}
            <div className="mt-4 grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
              <div className="rounded-xl border border-slate-200/60 bg-slate-50/70 p-4 shadow-inner dark:border-slate-800/80 dark:bg-slate-950/60">
                {auditLoading ? (
                  <div className="text-sm text-slate-500 dark:text-slate-400">Loading audit log...</div>
                ) : auditItems.length ? (
                  <AnimatedList
                    items={auditItems}
                    className="text-sm text-slate-700 dark:text-slate-200"
                    maxHeightClassName="max-h-[420px]"
                    onItemSelect={(item) => setSelectedAudit(item)}
                    renderItem={(item) => {
                      const details = item.details as Record<string, unknown> | undefined;
                      const username =
                        typeof details?.minecraft_username === "string" ? details.minecraft_username : null;
                      const label = item.summary || item.action.replace(/_/g, " ");
                      return (
                        <div className="rounded-lg border border-slate-200 px-3 py-2 dark:border-slate-700">
                          <div className="flex items-start justify-between gap-3">
                            ...
                            <span className="rounded-full border border-emerald-200 px-2 py-1 text-[11px] font-medium text-emerald-700 dark:border-emerald-700/50 dark:text-emerald-200">
                              {item.action}
                            </span>
                          </div>
                          {username && (
                            <div className="mt-2 text-xs text-slate-500 dark:text-slate-400">
                              Player: <span className="font-semibold text-slate-700 dark:text-slate-200">{username}</span>
                            </div>
                          )}
                        </div>
                      );
                  ...
                  <div className="text-sm text-slate-500 dark:text-slate-400">No audit events yet.</div>
                )}
              </div>
              <div className="rounded-xl border border-slate-200/60 bg-slate-50/70 p-4 shadow-inner dark:border-slate-800/80 dark:bg-slate-950/60">
                {selectedAudit ? (
                  <>
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="text-sm font-semibold text-slate-900 dark:text-slate-100">
                          {selectedAudit.summary || selectedAudit.action.replace(/_/g, " ")}
                        </div>
                        <div className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                    ...
                    <div className="mt-3 text-xs text-slate-500 dark:text-slate-400">
                      ...
                      <span className="font-semibold text-slate-700 dark:text-slate-200">
                        {selectedAudit.actor_email || "System"}
                      </span>
                    </div>
                    <div className="mt-3">
                      ...
                      <pre className="mt-2 max-h-64 overflow-auto rounded-lg border border-slate-200 bg-white p-3 text-xs text-slate-700 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200">
                        {JSON.stringify(selectedAudit.details ?? {}, null, 2)}
                      </pre>
                    </div>
                  </>
                ) : (
                  <div className="text-sm text-slate-500 dark:text-slate-400">Select an audit entry to view details.</div>
                )}
              </div>
            </div>
          </div>
