...
export function DragGallery({ items }: { items: DragGalleryItem[] }) {
  ...
  return (
    <div className="rounded-3xl border border-slate-800/60 bg-slate-950/70 p-4 sm:p-6 lg:p-10">
      ...
      >
        {loopedItems.map((it) => (
          <div
            key={it.key}
            className="drag-gallery-item min-w-[190px] flex-none rounded-2xl border border-slate-800/70 bg-slate-900/40 p-4 text-[13px] text-slate-300 sm:min-w-[260px] sm:p-6 sm:text-sm lg:min-w-[380px] lg:p-8 lg:text-lg"
          >
            {it.label}
            <div className="mt-3 h-[140px] overflow-hidden rounded-xl border border-slate-800/70 bg-slate-950/40 p-2 text-xs text-slate-500 sm:mt-4 sm:h-[200px] lg:h-[300px] lg:p-4">
              {it.imageUrl ? (
                <img
                  src={it.imageUrl}
